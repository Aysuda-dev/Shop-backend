...
def global_search(request):
    ...
    if query:
        products = Products.objects.filter(title__icontains=query)[:8]
        articles = Article.objects.filter(title__icontains=query)[:8]
        ...
        for article in articles:
            results.append({
                'title': article.title,
                'url': f'/article/detail/{article.slug}',
                'type': 'article',
                'image': article.images.url if article.images else None,
            })
...
def get_variant_detail(request, product_id):
    ...
    if not size_id or not color_id :
        return JsonResponse({"variant_id": None}, status=200)

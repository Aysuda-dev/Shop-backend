...
class RegisterView(View):
    ...
    def post(self,request):
        ...
        if register_form.is_valid():
            ...
            if user:
                register_form.add_error('email',_('کاربری با این ایمیل ثبت نام کرده است.لطفا ایمیل جدیدی وارد نمایید'))
            else:
                new_user=User(
                    email= user_email,
                ...
                messages.success(request, _('برای فعال سازی حساب، ایمیلی به آدرس شما ارسال شد.'))
    ...
    def get(self, request, email_active_code):
        ...
        if user :
            if not user.is_active:
                ...
                user.save()
                messages.success(request,_('حساب شما فعال شد'))
                return redirect(reverse('login_page'))
            else:
                messages.success(request,_('حساب شما فعال شد'))
                pass
...
class LoginView(View):
    ...
    def post(self,request:HttpRequest):
        ...
        if login_form.is_valid():
            ...
            if user :
                if not user.is_active:
                    login_form.add_error('email',_('حساب کاربری شما فعال نشده است'))
                    ...
                    if is_password_correct:
                        ...
                        login_form.add_error('password', _('رمز وارد شده اشتباه است'))

            else:
                login_form.add_error('email',_('کاربری با مشخصات وارد شده یافت نشد'))
        context= {
            'login_form': login_form
        }
        return render(request,'account_module/login_page.html',context)
...
class ForgotPasswordView(View):
    ...
    def post(self,request):
        ...
        if forgot_password_form.is_valid():
            ...
            user :User = User.objects.filter(email__iexact=user_email).first()
            if user:
                send_email(_('بازیابی کلمه عبور'), user.email, {'user': user}, 'emails/forgot_password.html')

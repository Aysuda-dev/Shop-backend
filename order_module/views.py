...
def addProductToOrder(request: HttpRequest):
    ...
    if count < 1:
        return JsonResponse({
            ...
            'text': _('مقدار وارد شده معتبر نمی باشد'),
            'confirm_button_text': _('مرسی از شما'),
            'icon': 'warning'
        })
    ...
    if request.user.is_authenticated:
        ...
        if variant:
            if count > variant.stock:
                return JsonResponse({
                    'status': 'out_of_stock',
                    'text': _("تعداد مورد نظر موجود نمیباشد"),
            ...
            return JsonResponse({
                'status': 'success',
                'text': _('محصول مورد نظر با موفقیت به سبد خرید شما اضافه شد'),
                'confirm_button_text': _('باشه ممنونم'),
                'icon': 'success'
            })
            ...
            return JsonResponse({
                'status': 'not_found',
                'text': _('محصول مورد نظر یافت نشد'),
                'confirm_button_text': _('مممنون'),
                'icon': 'error'
            })
        ...
        return JsonResponse({
            'status': 'not_auth',
            'text': _('برای افزودن محصول به سبد خرید ابتدا می بایست وارد سایت شوید'),
            'confirm_button_text': _('ورود به سایت'),
            'icon': 'error'
        })
...
def request_payment(request: HttpRequest):
    ...
    for detail in current_order.orderdetail_set.all():
        if detail.count > detail.variant.stock:
            return JsonResponse({
                'status': 'out_of_stock',
                'text': _("موجودی برخی از محصولات تغییر کرده لطفا بعد از اصلاح سبد خرید خود اقدام به پرداخت کنید"),
                'icon': 'error'
            })
